...
$sql = "SELECT * FROM influenza_vaccination WHERE 1";

if ($yearFilter !== null) {
    $sql .= " AND YEAR(vaccination_date) = $yearFilter";

    if ($monthFilter !== null) {
        // Display residents registered in the selected month and year
        $sql .= " AND MONTH(vaccination_date) = $monthFilter";
    }
}

$sql .= " ORDER BY vaccination_date DESC";
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo '<script>var influenzaData = ' . json_encode($rows) . ';</script>';
    ...
    for ($i = $startIndex; $i < $endIndex; $i++) {
        ...
        echo "<option value='view/services4.php?update=".$row['id']."'>Update</option>";
        echo "<option value='view/delete_services4.php?id=".$row['id']."'>Delete</option>";
        echo "</select> </th></tr>";
        $even++;
    }
