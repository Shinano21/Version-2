...
if (isset($_POST["submit"])) {
    ...
    $sqlUsers = "SELECT `password` FROM `administrator` WHERE `unique_id` = ?";
    ...
    if (mysqli_stmt_num_rows($stmtUsers) > 0) {
        ...
        if (password_verify($old_pass, $hashedPassword)) {
            ...
            $sqlUpdateUsers = "UPDATE `administrator` SET `password` = ? WHERE `unique_id` = ?";
            ...
            if (mysqli_stmt_affected_rows($stmtUpdateUsers) > 0) {
                ...
                echo 'window.location.href = "edit_admin_profile.php";';  // Optional: Redirect after displaying the alert
                echo '</script>';
                exit();
            } else {
